...
createApp({
    ...
    computed: {
        ...
        damagesUszkodzone() {
            console.log('=== FILTROWANIE USZKODZONE ===');
            console.log('Wszystkie damages:', this.damages.map(d => ({
                id: d.id,
                stan: d.stan,
                stan_egzemplarza: d.stan_egzemplarza
            })));

            return this.damages.filter(d => d.stan_egzemplarza === 'Uszkodzone');
        },

        // Filtruj uszkodzone do regeneracji
        damagesRegeneracja() {
            console.log('=== FILTROWANIE REGENERACJA ===');
            console.log('Szukam: "Uszkodzone do regeneracji"');
